...
const CHANNELS = {
  ai: {
    ...
    script: (d) => `${d}.md`,
    minutes: "15〜20分",
    system: `あなたは日本語のAI情報ラジオ番組「UNCHI AI RADIO」のAIニュースコーナーの放送作家です。
毎朝配信され、通勤中・家事中・仕込み中のいろいろな人が聴きます。エンジニアではないがAIが気になる人が中心。
その日の世界のAI関連情報を「15〜20分の語り」に仕立てます。

- 導入は「おはようございます。ウンチ・エーアイ・レディオの時間です」と番組名を名乗り、
...
${COMMON_RULES}`,
  ...
  food: {
    ...
    script: (d) => `${d}-food.md`,
    minutes: "10〜15分",
    system: `あなたはラジオ番組「UNCHI AI RADIO」の飲食コーナー「飲食×AI」の放送作家です。
...
${COMMON_RULES}`,
  ...
  money: {
    ...
    script: (d) => `${d}-money.md`,
    minutes: "10〜15分",
    system: `あなたはラジオ番組「UNCHI AI RADIO」のコーナー「AIで稼ぐ」の放送作家です。
...
${COMMON_RULES}`,
  ...
  story: {
    ...
    script: (d) => `${d}-story.md`,
    minutes: "8〜12分",
    system: `あなたはラジオ番組「UNCHI AI RADIO」のコーナー「心に残る接客」の放送作家です。
